...
export class NativeProvider {
    ...
    private startCamera() {
        this.camera.getPicture(this.cameraOpt).then((imageData) => {
            ...
            this.utilService.showToast(this.toastCtrl, '无法使用拍照功能'); //错误：无法使用拍照功能！
        });
    }
    ...
    private openImgPicker() {
        ...
        this.imagePicker.getPictures(this.imagePickerOpt)
            .then((results) => {
                ...
                this.utilService.showToast(this.toastCtrl, '无法从手机相册中选择图片'); //错误：无法从手机相册中选择图片！
            });
        ...
        this.fileTransfer.upload(path, this.upload.url, options)
            .then((data) => {
                ...
                if (this.upload.error) {
                    ...
                    this.utilService.showToast(this.toastCtrl, '错误：上传失败！');
                }
            });
    }
